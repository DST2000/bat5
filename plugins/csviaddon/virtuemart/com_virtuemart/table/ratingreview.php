 ...
 * @since       6.0
 */
class VirtueMartTableRatingReview extends CsviTableDefault
{
	/**
	 * Table constructor.
	 *
	...
	{
		parent::__construct('#__virtuemart_rating_reviews', 'virtuemart_rating_review_id', $db, $config);
	}
	...
	{
		if (empty($this->virtuemart_rating_review_id))
		{
			...
			$this->db->execute();
			$this->log->add('Check if a rating review exists');

			if ($this->db->getAffectedRows() > 0)
			{
				$this->virtuemart_rating_review_id = $this->db->loadResult();
	...
	public function reset()
	{
		...
		$this->virtuemart_rating_review_id = null;
